...
#notetickの重複を排除
def notesDeduplication(noteList):
    newNoteList = []
    lastNote = [[-999,-1,-1],[-999,-1,-1],[-999,-1,-1],[-999,-1,-1]]
    for L in noteList:
        if not(lastNote[L[1]][0] - 50 <= L[0] <= lastNote[L[1]][0] + 50):
            newNoteList.append(L)
        lastNote[L[1]] = L
    return newNoteList
        
def longNotesDeduplication(noteList):
    lastNote = [-1,-1,-1,-1]
    for i in range(0,int(len(noteList))):
        if noteList[lastNote[noteList[i][1]]][0] + noteList[lastNote[noteList[i][1]]][2] >= noteList[i][0] - 75:
            noteList[lastNote[noteList[i][1]]][2] = noteList[i][0] - noteList[lastNote[noteList[i][1]]][0] - 75
            if noteList[lastNote[noteList[i][1]]][2] < 0:
                noteList[lastNote[noteList[i][1]]][2] = 0
        lastNote[noteList[i][1]] = i
    return noteList
...
noteList = notesDeduplication(noteList)
noteList = longNotesDeduplication(noteList)
writeInfo(fpOut,noteList)
